...
const Calendar = ({date}) => {
      ...
      let firstDayOfMonth = date.getDay() - 1;
       ...
       while (firstDayOfMonth > 0) {
            monthInfo.push({day: lastDayOfPreviousMonth.getDate() - firstDayOfMonth + 1, week: 1, dayClass: 'ui-datepicker-other-month'});
            --firstDayOfMonth;
        }
      for(let i = 1; i < lastDayOfMonth.getDate() + 1; ++i) {
          if (i === date.getDate()) {
              monthInfo.push({day: i, week: Math.ceil((monthInfo.length + 1) / 7), dayClass: 'ui-datepicker-today'});
          } else {
            monthInfo.push({day: i, week: Math.ceil((monthInfo.length + 1) / 7), dayClass: ''});
          }
      }
      let lastDayOfWeek = 1;
      while (monthInfo.length < 7 * monthInfo[monthInfo.length - 1].week) {
        monthInfo.push({day: lastDayOfWeek++, week: monthInfo[monthInfo.length - 1].week, dayClass: 'ui-datepicker-other-month'});
    }
